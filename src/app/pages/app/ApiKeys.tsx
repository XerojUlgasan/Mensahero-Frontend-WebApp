...
function normalizeStatus(status: string): ApiKey["status"] {
...
export function ApiKeys() {
  ...
  useEffect(() => {
    ...
    if (cached && !hydratedRef.current) {
      setKeys(cached.map(mapRetrievedKey));
    ...
    const loadKeys = async () => {
      try {
        ...
        if (hasDataChanged(cached, next) || !hydratedRef.current) {
          setKeys(next.map(mapRetrievedKey));
          hydratedRef.current = true;
        }
      } catch (error) {
        if (!cached) {
  ...
  function mapRetrievedKey(key: RetrievedApiKey): ApiKey {
    return {
      id: key.id,
      name: key.name,
      maskedValue: maskSecret(key.key),
      fullValue: key.key,
      created: formatDateLabel(key.created_at),
      lastUsed: formatDateLabel(key.last_used),
    ...
    <div style={{ padding: 32 }}>
      ...
      {showCreate && (
        ...
        >
          ...
          >
            <p
              style={{ color: "var(--mh-text)", fontWeight: 600, fontSize: 14 }}
            >
              New API Key
            </p>
        ...
        >
          ...
          >
            ...
            <button
              ...
              style={{
                ...btnBase,
                color: copiedIds.has("created") ? "#15803D" : "#6B7280",
              }}
            >
              {copiedIds.has("created") ? (
                <Check size={15} />
              ) : (
                <Copy size={15} />
              )}
      ...
      {keys.length === 0 ? (
        ...
        >
          ...
          <p
            style={{ color: "var(--mh-muted)", fontSize: 14, marginBottom: 20 }}
          >
          ...
          {keys.map((key, i) => (
            <div
              ...
              style={{
                padding: "20px 24px",
                borderBottom:
                  i < keys.length - 1 ? "1px solid var(--mh-border)" : "none",
              }}
            >
              <div
                style={{
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...btnBase,
                        padding: "2px 4px",
                        color: copiedIds.has(key.id)
                          ? "var(--mh-green)"
                          : "var(--mh-muted)",
                      }}
                      title="Copy"
                    >
                      {copiedIds.has(key.id) ? (
                        <Check size={14} />
                      ) : (
                        <Copy size={14} />
                      )}
